...
Protected Module Globals
	...
	#tag Method, Flags = &h0
		Function GetUserHelpTagForChannel(Username As String, Product As UInt32, Flags As UInt32, Ping As UInt32, Statstring As String) As String
		  
		  Dim sTmp As String
		  Dim dwTmp1 As UInt32
		  
		  sTmp = "Username:  " + Username + EndOfLine + _
		  "Product:  " + Globals.ProductName(Product) + EndOfLine + _
		  "Flags:  0x" + MemClass.HexPrefix(Flags) + _
		  " (" + Globals.SSimpleUserFlags(Flags) + ")" + EndOfLine + _
		  "Ping:  " + Str(Ping) + "ms"
		  
		  If Globals.IsWarcraftIII(Product) = True And CountFields(Statstring, " ") >= 4 Then
		    dwTmp1 = MemClass.ReadDWORD(NthField(Statstring, " ", 4), 1, True)
		    sTmp = sTmp + EndOfLine + "Clan:  " + Globals.SClanTag(dwTmp1)
		    
		  ElseIf Globals.IsStarcraft(Product) = True Or Globals.IsWarcraftII(Product) = True Then
		    dwTmp1 = Val(NthField(Statstring, " ", 4))
		    sTmp = sTmp + EndOfLine + "Normal Game Wins: " + Str(dwTmp1)
		  End If
	...
	#tag Method, Flags = &h0
		Function GetUserHelpTagForClan(Username As String, Rank As Byte, LocationType As Byte, Location As String) As String
	...
	#tag Method, Flags = &h0
		Function GetUserHelpTagForFriends(Username As String, Product As UInt32, Status As Byte, LocationType As Byte, Location As String) As String
		  ...
		  End Select
		  sTmp = Replace(sTmp, " ()", "") + EndOfLine + "Status:  "
		  
		  If BitAnd(Status, &H01) > 0 Then sTmp = sTmp + "Mutual, "
		  If BitAnd(Status, &H02) > 0 Then sTmp = sTmp + "DND, "
		  If BitAnd(Status, &H04) > 0 Then sTmp = sTmp + "Away, "
		  If BitAnd(Status, &HF8) > 0 Then sTmp = sTmp + "& Unknown, "
		  If Status = 0 Then sTmp = sTmp + "n/a, "
	...
	#tag Method, Flags = &h0
		Function SSimpleUserFlags(Flags As UInt32) As String
		  
		  Dim sTmp As String
		  
		  If BitAnd(Flags, &H01) > 0 Then sTmp = sTmp + "Blizzard Admin, "
		  If BitAnd(Flags, &H08) > 0 Then sTmp = sTmp + "Battle.net Admin, "
		  If BitAnd(Flags, &H02) > 0 Then sTmp = sTmp + "Channel Op, "
		  If BitAnd(Flags, &H04) > 0 Then sTmp = sTmp + "Channel Speaker, "
		  If BitAnd(Flags, &H20) > 0 Then sTmp = sTmp + "Squelched, "
		  If BitAnd(Flags, &H40) > 0 Then sTmp = sTmp + "Guest, "
		  If BitAnd(Flags, &H10) > 0 Then sTmp = sTmp + "No UDP, "
		  
		  If Right(sTmp, 2) = ", " Then sTmp = Left(sTmp, Len(sTmp) - 2)
		  IF Len(sTmp) = 0 Then sTmp = "Normal"
